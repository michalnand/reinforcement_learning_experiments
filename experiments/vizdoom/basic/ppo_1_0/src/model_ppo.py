...
class Model(torch.nn.Module):
    ...
    def forward(self, state, hidden_state):
        ...
        value   = self.model_critic(z)

        return policy, value, hidden_state_new  

    def forward_features(self, state):
        s        = state[:, 0:self.in_ch, :, :].unsqueeze(1)
        features = self.model_features(s)
...
if __name__ == "__main__":
    ...
    policy, value, hidden_state_new = model(state, hidden_state)

    print("shape = ", policy.shape, value.shape, hidden_state_new.shape)
